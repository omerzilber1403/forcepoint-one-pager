  ...
  return [
    {
      ...
      body: isHe ? (
        <div className="spl-body">
          ...
          <p><strong style={{ color: S }}>גישור DB ב-Python (שכבת אמצע):</strong>{" "}שרת Python המשמש כגשר מעל Raw TCP בפורט <code style={LC}>7778</code>, ומבצע שאילתות SQL מול מסד נתונים <code style={LC}>SQLite</code>. מנהל רישום משתמשים, לוגי התחברויות, ושמירת לוגי אירועים — מה שמשאיר את שרת ה-Java במצב Stateless.</p>
          <p><strong style={{ color: S }}>קליינט C++ (שכבה תחתונה):</strong>{" "}ממומש ב-C++, ומשמש כממשק <code style={LC}>STOMP 1.2</code> בצד הלקוח עבור הרשמה לערוצים, דיווח אירועים, ויצירת סיכומים.</p>
        </div>
      ) : (
        <div className="spl-body">
    ...
    },
    {
      ...
      body: isHe ? (
        <div className="spl-body">
          ...
          <p><strong style={{ color: S }}>תהליכון המקלדת (Keyboard Thread):</strong>{" "}קורא פקודות בלעדית מ-<code style={LC}>stdin</code> וכותב STOMP Frames יוצאים אל ה-Socket. תהליכון זה לעולם לא קורא מה-Socket — הבעלות (Ownership) מוגדרת באופן נוקשה.</p>
    ...
    },
    {
      ...
      body: isHe ? (
        <div className="spl-body">
          ...
          <p><strong style={{ color: S }}>מזהים מבוססי-לקוח (Client-generated IDs):</strong>{" "}מזהי הרשמות (Subscription IDs) ומזהי קבלות (Receipt IDs) מיוצרים בצורה ייחודית על ידי הלקוח ומנוהלים מקומית. השרת מהדהד אותם חזרה ב-<code style={LC}>MESSAGE</code> ו-<code style={LC}>RECEIPT</code> Frames לחיפוש O(1) בצד הלקוח.</p>
          <p><strong style={{ color: S }}>פענוח (Frame Parsing):</strong>{" "}קורא את זרם ה-Raw Socket, מפענח צמדי <code style={LC}>header:value</code> שורה אחר שורה, ומחלץ את גוף ההודעה (Body) עד לתו מסיים המחרוזת (<code style={LC}>\0</code>).</p>
        </div>
      ) : (
        <div className="spl-body">
    ...
    },
    {
      ...
      body: isHe ? (
        <div className="spl-body">
          ...
          <p><strong style={{ color: S }}>סידור כרונולוגי:</strong>{" "}האירועים נשמרים ומודפסים לפי סדר התרחשותם. מיון משני על רשימת האירועים לפי שדה הזמן (<code style={LC}>time</code>) מייצר את הפלט הסופי.</p>
    ...
    },
    {
      ...
      body: isHe ? (
        <div className="spl-body">
          ...
          <p><strong style={{ color: S }}>אישור שרת (Server Acknowledgment):</strong>{" "}הקליינט ממתין באופן נוקשה ל-RECEIPT Frame התואם לפני קריאה ל-<code style={LC}>close(sockfd)</code>. תהליכון המקלדת נחסם על Condition Variable עד שתהליכון ה-Socket מאותת שהקבלה אושרה.</p>
...
/* ─── LightSPLCaseStudy ──────────────────────────────────────────────────── */
export default function LightSPLCaseStudy({ lang = "en" }: { lang?: Lang }) {
  const isHe = lang === "he";
        ...
        <div className="spl-context" dir={isHe ? "rtl" : undefined}>
          <div className="spl-context-card">
            ...
            <div className="spl-context-card-body">
              {isHe ? (<>מערכת <strong>Pub/Sub בזמן אמת</strong> לאירועי משחקי כדורגל. כתבים מעלים קבצי JSON של אירועי משחק לערוץ ייעודי (למשל: <code style={LC}>argentina_france</code>). כל קליינט שמנוי לערוץ מקבל באופן מיידי MESSAGE frame על כל אירוע — שערים, בעיטות פתיחה, חילופים — ברגע שהם מפורסמים.</>) : (<>A <strong>real-time pub/sub event system</strong> for football matches. Reporters upload game event JSON files to a named channel (e.g. <code style={LC}>argentina_france</code>). Every subscribed client instantly receives a MESSAGE frame per event — scores, kickoffs, substitutions — as they are published.</>)}
            </div>
          </div>
          <div className="spl-context-card">
            <div className="spl-context-card-label">{isHe ? "מי משתמש בזה?" : "Who uses it"}</div>
            <div className="spl-context-card-body">
              {isHe ? (<>שני סוגי משתמשים (Roles) חולקים את אותו השרת. <strong>כתב (Reporter)</strong> מתחבר, נרשם לערוץ, ומעלה קובץ אירועים. <strong>מנוי (Subscriber)</strong> מתחבר ונרשם כדי לקבל עדכונים בלייב. כל קליינט יכול לקרוא לפקודת <code style={LC}>summary</code> כדי לקבל נתונים סטטיסטיים מרוכזים עבור כל כתב, בכל ערוץ.</>) : (<>Two user roles share the same server. A <strong>reporter</strong> connects, subscribes to a channel, and uploads an event file. A <strong>subscriber</strong> connects and subscribes to receive live updates. Any client can call <code style={LC}> summary</code> to get aggregated stats for any reporter on any channel.</>)}
            </div>
          </div>
          <div className="spl-context-card">
